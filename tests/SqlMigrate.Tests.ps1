...
Describe "Set-FileMigrated" {
    ...
    It "should correctly save execution time" {
        $TestTimeTicks = 1000
        Set-FileMigrated -Connection $Connection -FileName $FileName -ExecutionDuration ([TimeSpan]::FromTicks($TestTimeTicks))
        (Invoke-SQL -ConnectionString $testDatabaseConnectionString -sqlCommand "Select Top 1 execution_duration As Duration From $HistoryTable").Duration | Should Be $TestTimeTicks
    }
